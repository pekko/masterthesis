...
However, the Jaccard Index takes into account only size of intersection, and not the fact how popular an element was in the dataset. It does not matter whether the two nodes had the most popular SSID in common, or if they were the only two nodes that had the SSID. In the paper, Cunche et al. argue that having a rare SSID should yield for stronger possibility of relationship. That is a reasonable argument, especially when we take into account the long tail of SSID frequency: a few SSIDs are extremely common, but huge majority of them appear only in a few devices.
...
where q is a parameter and $f_z$ is the frequency of the element $q$. The authors obtained best results when $q=3$.~\cite{cunche2014linking}
