...
We can approach the problem in two ways. First, we use coordinates and distance. If a user has an SSID which maps to location $(x_0, y_0)$, search for all the tweets with coordinates $(x_1, y_1)$ where distance 
$d\left( (x_0,y_0), (x_1,y_1) \right)$ 
is smaller than certain search radius $r$, a parameter. After that, count the number of locations that matched. If the number is large enough -- threshold given as a parameter -- this might be a match. First verification can be to check if the user was nearby capture location at capture time, or somewhere else.
...
The first is considered error-free: we have succesfully captured possible SSID probes, and filtered erroneous ones out with the checksum field (FCS) of the 802.11 standard. Coverage could be increased with time, better antennas, and better capture positions, such as larger cities.
