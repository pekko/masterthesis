...
\usepackage[twoinstructors]{aalto-thesis}
...
% ------------------------------------------------------------------
\newcommand{\TITLE}{Caught Before Connecting}
\newcommand{\FTITLE}{Havaittu ennen yhdist\"{a}mist\"{a}}
\newcommand{\SUBTITLE}{Privacy Implications of Wireless Network Probing}
\newcommand{\FSUBTITLE}{Yksityisyydensuoja langattomia verkkoja etsiess\"{a}}
\newcommand{\DATE}{October 10, 2014}
\newcommand{\FDATE}{10. lokakuuta 2014}
...
\newcommand{\KEYWORDS}{wireless networks, probing, privacy, profiling}
\newcommand{\FKEYWORDS}{langattomat verkot, yksityisyys, profilointi}
...
Manufacturers want to make wireless devices to join network fast. In order to do that, devices often remember the networks joined previously, and will try these first by probing them. If that fails, the device will request list of all available networks in the range, or passively monitor to see the networks.

In this thesis we recorded half a million wireless network probe requests, and many of them included names for previously-joined networks. After that, we combined the list with publicly accessible data, and tried to figure out where a specific network is located; if it belongs to, for example, a hotel or a restaurant; and how expensive the hotel or the restaurant is supposed to be. Combining the list of networks with network information may reveal device owner's hometown, willingness to pay for high-end services, or other behavioral patterns.

We also studied the legal status of the issue, and found out that in the U.S. such profiling is legal. However, in Europe the laws are stricter, but no definitive answer can be given: in order to apply restrictions, legislation requires that an individual must be able to identify, which we tried with social media data, but did not see reasonably possible.

There are also numerous ways to perform active attacks, which do not limit to profiling user's behavior, but are suitable for malicious intents such as eavesdropping user's web browsing. In addition, there are easy-to-use tools available on the market, and performing an attack does not require advanced knowledge.

To prevent user's privacy against the profiling techniques, we looked for possible countermeasures, their effectiveness, and feasibility to implement. There are promising ones available, and first manufacturers are implementing them. Also, the simplest defense is to disable active probing, which would be a great way to stop most of information leaks, but not all.
}

% Abstract in Finnish
% argh latex, plase support utf-8
% ------------------------------------------------------------------
\thesisabstract{finnish}{
Valmistajat optimoivat laittensa liittym\"{a}\"{a}n langattomiin verkkoihin nopeasti. T\"{a}st\"{a} syyst\"{a} laitteet pit\"{a}v\"{a}t kirjaa verkoista, joihin ovat aiemmin liityneet, ja yritt\"{a}v\"{a}t ensin etsi\"{a} niit\"{a}. Ep\"{a}onnistuessaan laite voi pyyt\"{a}\"{a} tietoa kaikista alueella olevista verkoista tai passivisesti etsi\"{a} uusia.

T\"{a}ss\"{a} diplomity\"{o}ss\"{a} tallensimme puoli miljoonaa langattoman verkon etsint\"{a}yrityst\"{a}, joista monet sis\"{a}lsiv\"{a}t aiemmin liitytyn verkon nimen. Yhdistimme tulokset avoimiin tietokantoihin, ja yritimme selvitt\"{a}\"{a} miss\"{a} kyseiset verkot fyysisesti sijaitsivat, kuuluivatko ne esimerkiksi jollekin hotellille tai ravintolalle, ja mihin hintaluokkaan kyseinen hotelli, ravintola tai muu palvelu kuuluu. T\"{a}m\"{a}nkaltainen tietojen yhdist\"{a}minen voi paljastaa kiinnostavia tietoja laitteen omistajasta, kuten kotipaikan tai valmiuden maksaa korkealaatuisesta palvelusta.

Tutkimme my\"{o}s juridista n\"{a}k\"{o}kulmaa profilointiin. Havaitsimme, ett\"{a} Yhdysvalloissa se on hyvin pitk\"{a}lti laillista. Euroopan Unionissa tietosuojam\"{a}\"{a}r\"{a}ykset ovat merkitt\"{a}v\"{a}sti tiukemmat, mutta t\"{a}sm\"{a}llist\"{a} ja varmaa vastausta emme voi antaa: rajoitukset edellytt\"{a}v\"{a}t, ett\"{a} yksitt\"{a}inen henkil\"{o} voidaan tunnistaa datasta, mit\"{a} yritimme sosiaalisen median avulla, mutta emme pid\"{a} k\"{a}yt\"{a}nn\"{o}ss\"{a} mahdollisena.

Passiivisen seurannan ja profiloinnin lis\"{a}ksi tutkimme aktiivisia hy\"{o}kk\"{a}yksi\"{a}. N\"{a}m\"{a} menev\"{a}t pidemm\"{a}lle ja ovat vaarallisempia kuin edell\"{a} kuvattu seuranta, sill\"{a} ne voivat onnistuessaan kaapata ja muokata kohteen web-selaamista. Markkinoilla on my\"{o}s valmiita ja helppok\"{a}ytt\"{o}isi\"{a} ty\"{o}kaluja, joten hy\"{o}kk\"{a}ykseen ei tarvita erityisen edistyksellisi\"{a} taitoja.

Yksityisyyden turvaamiseksi tutkimme mahdollisia vastatoimenpiteit\"{a} edell\"{a} kuvattuja tekniikoita vastaan, ja erityisesti n\"{a}iden tehokkuutta ja toteutettavuutta. L\"{o}ysimme lupaavia ehdokkaita, ja havaitisimme, ett\"{a} ensimm\"{a}iset valmistajat ovat jo ottamassa n\"{a}it\"{a} k\"{a}ytt\"{o}\"{o}n. Yksinkertaisin suojautuminen, aktiivisen etsimisen kytkeminen pois p\"{a}\"{a}lt\"{a}, est\"{a}isi valtaosan tietovuoidoista -- vaan ei kaikkia.
}
...
Last, I wish to thank all my friends in Finland and around the world for a great student life, and especially the ones who gave me a push to go international.
...
\begin{figure}
    \includegraphics[width=0.9\textwidth]{images/wps}
...
The UUID is Universally Unique Identifier, defined by RFC 4122~\cite{rfc4122}. Depending on the purpose, UUID can be time-based, name-based or random-based. As the names suggest, time-based UUIDs include creation timestamp, whereas name-based ones remain constant for the same entity, for example for the same device creating them. The name used in UUIDs can be any kind of object identifier, for instance a MAC address or a domain name.

In WPS, we found that the devices use version 5 of UUIDs, which is a name-based one. The UUID is formed by hashing a namespace UUID and device's identifier using SHA-1 algorithm. We cannot retrieve the identifier used while generating the UUID, but we consider it as a information leak: depending on the implementation, it might be possible that UUID remains constant even if some other identifiers are changed, such as while randomizing the MAC address.

Thus, with all this information, we may be able to
\begin{enumerate}
...
In active attacks, the attacker is usually able to conduct greater violations to victim's privacy than only profiling or tracking the user. The attacker might be able to read all the victim's communication or alter it. However, the initial cause of privacy violation remains the same: the user is transferring data to a party it did not suppose to. In previous cases, the unwanted party was general public; now it is the single attacker.
...
If it is, then all purposes of where data is used must have been told to the data subject -- that is, the target of collection -- beforehand, and his consent must be obtained. Also, there is so-called ``right to be forgotten'', which includes a possibility for everyone to see their personal data and ask for its deletion in the Data Protection Directive, Article 12~\cite{data_protection}. 

However, if the data is not considered to be personal data, no EU-wide legislation applies. 

\subsection{Wiretapping laws}

In addition to privacy laws, there might be an issue with legislation on wiretapping. These laws vary between countries, and the European Union seems not to have a clear directive on the issue. In the directive 97/66/EC~\cite{privacy_directive}, Article 5, the directive defines confidentiality of communication as follows:
\begin{quote}
    [Member states] shall prohibit listening, tapping, storage or other kinds of interception or surveillance of communications, by others than users, without the consent of the users concerned
\end{quote}

Thus, the problem here is to define \emph{communications}. Clearly monitoring and recording actual traffic, for example web browsing, is illegal. However, it is unclear if only probing different networks should be considered as communications. The directive is about ``public telecommunications network'', which means (emphasis added)
\begin{quote}
    transmission systems and, where applicable, switching equipment and other resources which permit the conveyance of signals between \textbf{defined termination points} [..]
\end{quote}

When an access point is broadcasting its bacons, there is only a single termination point, the access point itself -- the targeted audience of probes are everyone. Thus, it should not be considered as communications in the directive.

When a client device is probing the SSIDs it has, the situation is more difficult. Once again, the probe is technically broadcasted, but now it includes also the target SSID. Thus, by strictly interpreting the definition, it might be possible that there are two defined termination points, and it should be treated as communications, and recording them would be illegal.
...
\center
    \begin{tabular}{|l|c|c|}
        ...
        \hline
        Collecting SSIDs from mobile devices & Possibly illegal & Legal \\ 
        \hline
...
However, we showed that it is possible to form behavioral patterns based on SSID list. We successfully ran a graph clustering algorithm, which showed plausible links between networks and companies. We were successful to discover user's possible hometown. By using third-party services, we also created our hypotheses on users' willingness to pay for high-end services, which would be extremely valuable for business-owners.
...
$$D = \frac{K}{N} = \frac{2^{24}}{8} \approx 2 \times 10^6$$
