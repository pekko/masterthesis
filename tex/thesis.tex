...
\newcommand{\DATE}{September 15, 2014}
\newcommand{\FDATE}{15. syyskuuta 2014}
...
Wireless devices are optimized to join networks fast. In order to do that, they often remember the networks joined previously, and will try these first by probing them. If that fails, a device will request list of all available networks in the range, or passively monitor to see the networks.

In this thesis we recorded half a million of wireless network probe requests, and many of them included names for previously-joined networks. After that, we combined the list with publicly-accessible data, and tried to figure out where a specific network is located, if it belongs to, for example, a hotel or a restaurant, and how expensive the hotel or the restaurant is supposed to be. Combining list of networks with network information may reveal device owner's hometown, willingness to pay for high-end services, or other behavioral patterns.
...
Last, I wish to thank my friends around the world for a great student life, and especially the ones in Finland giving a push to go international.
...
IEEE Standard states wildcard SSID to be the one with length set to zero. 
