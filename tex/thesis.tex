...
We collected total of 279,950 valid probe requests from 12,856 different devices. Here, we assume that devices are following the standard and have an unique and permanent MAC address. 9,872 devices (76.8~\%) probed only wildcard SSID, and these were excluded from later analysis. Therefore, we have 2,984 unique devices left.

We saw 8,454 unique SSIDs. Distribution of nodes per SSID is shown on Figure~\ref{fig:nodes_per_ssid}. We see that huge majority of SSIDs, 7,927 or 86~\%, were found only on a single device. This will make it harder to find clusters, but allows to differentiate users more.
...
In Figure~\ref{fig:ssids_per_node} we see how many SSIDs a node probed. There is a bias on one ssid per node, since in many cases it may be result of joining a network. Since we did not collect any other data than probe requests, this hypothesis is impossible to verify. After that, most of the clients probed at most 17 SSIDs.

\begin{figure}
    \center
    \includegraphics{images/ssids_per_node}
    \caption{How many nodes did a client probe. Note logarithmic scale.}
...
\section{Comparing devices with each others}
...
\begin{enumerate}
    ...
    \item Calculate score for uncertainity of the location data:
        $$ u_{Loc} = \ln a + d^2 $$
        where $a$ is number of different locations for the SSID, and $d$ is the average distance between locations.
    
    \item By location data and SSID, query Google Places to find out what place this is. SSID was splitted into different words by different delimiters, such as `-', `\_', whitespace, and case changes. For example, SSID \texttt{FourSeasons\_Guests} splits into search terms \texttt{Four}, \texttt{Seasons} and \texttt{Guests}.

    \item If the SSID or data from Google suggested it to be a hotel, query Expedia database to get more details.

    \item Last, manually assign uncertainty score for Google Places result.
\end{enumerate}

In Table~\ref{tab:stage_counts} there are count of SSIDs processed in every phase. We see from the table, that even though we were able to get location infromation for almost two-thirds of SSIDs, only one third of the locations were considered accurate enough. This is due to ambiguous SSIDs: they appear more than once and in various locations.
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|c|c|}
        ...
        Detailed hotel information              & 389  & 5~\%   \\ \hline
    \end{tabular}
...
\begin{itemize}
    \item found 20 ssids of form \texttt{firstname lastname's [something]}
    \begin{itemize}
        \item .. three of them were \texttt{.. guest network}
        \item owner != user
    \end{itemize}
    \item 33 WPS leaks
    \begin{itemize}
        \item sadly default names.. motorola, verizon!
    \end{itemize}
\end{itemize}
