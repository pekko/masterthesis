...
\textbf{How much is a user travelling?} Identify airports and hotels. Categorize user to different price categories based on hotel's average price level.
...
In this chapter, we will conduct a practical experiment of previously discussed techniques and methods. TODO more.

\section{Methodology}
\label{sec:methods}

Data was captured from various locations using a single laptop with an internal wireless antenna. No special or expensive equipment was used.
...
Twitter was a special case, since its API is pretty limited for rate. We collected Twitter's feed to our own servers and built our own database, so that we gate an unlimited access to data. Unfortunately, because Twitter's feed is so huge, we were able to collect only 10~\% of it. Therefore, private access to Twitter's full database would give better results.
