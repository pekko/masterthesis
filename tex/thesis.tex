...
\begin{tabular}{c|c p{10cm}}
    ...
    \hline
\end{tabular}
\caption{TODO Draw prettier one: Protocol for joining a WPS-enabled WLAN. Simplified from~\cite{microsoftWCN}}.
\end{figure}
...
In Figure~\ref{fig:ssids_per_node} we see how many SSIDs a node probed. Almost half of the nodes, 48~\%, probed only one SSID and are exluded from the graph. This behavior may be result of joining a network. Since we did not collect any other data than probe requests, this hypothesis is impossible to verify. After that, we see various number of probes between two and 15, but on sixteen nodes there is a very significant raise. As $16 = 2^4$, we believe that this is hard-coded limit on some devices. However, no single device manufacturer stand up: Apple, Samsung, LG and others are all presented in expected way in nodes with SSID count of sixteen.
...
We tried to find clusters from the data by using various methods. By finding clusters, we were looking if we could discover patterns. For example, do some SSIDs appear together often?

First, we calculated the similarity matrix of nodes by looping through all pairs. For similarity metric, we used Jaccard Index and Psim-3, as specified in Section~\ref{subsec:clustering}. After that, we run the actual clustering algorithm, and interpreted results by forming a visual representation or connecting clusters with raw data. Visualization gives us overview how the data looks like, whereas connecting with data gives exact information.

Hierarchical clustering does not work well for this type of data. Too many nodes had exactly zero similarity, which yield to maximum distance (in hierarchical clustering, we need to use distance instead of similarity).

Markov Clustering (MCL) works better with the data. We first created clusters of client devices with MCL, and then looked at shared SSIDs in each cluster. SSIDs were scored and sorted based on the frequency they appeared in a cluster. 

First there are seemingly obvious connections, such as people using differently named networks within the same university or another organization, such as \texttt{UCSB Secure} and \texttt{UCSB Wireless Web}. By these, we can verify that our algorithm works at least on some extent, and can continue for further results. We see that there are definetely links between companies: we identified at least a few different restaurants, ice cream bars, hair salons and retail stores. Once again, the problem is the long tail of SSIDs: most of them appeared only in a small number of devices.

To obtain a graphical representation of the data, we used Gephi and its \emph{Force Atlas} method~\cite{bastian2009gephi} to give graphical representation of the data. Showing all the edges was too heavy task, so we filtered the least important out. With Prism-3 similarity method and threshold$= 0.1$, we got the result shown in Figure~\ref{gephi_01}.
...
It would be very valuable to know is users are willing to pay more, or if they are looking for the cheapest alternative. We can try to fetch a rough approximation from SSID list: it is a reasonable hypothesis, that if the user has visited a lot of expensive hotels and restaurants, she is ready to spend money on high-end products.

We fetched pricing information from Yelp~\cite{yelp}, which reports price range for places with four-step scale, from one to four. This is a helpful feature, because then we need not to normalize the prices between different cities or different types of services (such as hotels or restaurants).
...
The distribution of price average is shown in Figure~\ref{fig:prices_users}. We see a big peak on range 2.0 -- 2.49, and only five users had an average of at least 3.0. This is may be a result of nature of Yelp data. Nonetheless, we believe that in many cases the result gives hints about customer's purchasing power or consuming behavior, accuracy correlating with the number of SSIDs where pricing data was found.
...
One task was to find out where a user comes from. Our hypothesis was that most of the people are from California, where the data was collected, but significant number of people probably are foreigners due to nature of area. Here, we define hometown as a place where user has spent most of his time lately: that is, if a German is visiting U.S., his hometown is still in German -- but if he has lived in California for a year, his current hometown can be considered to be in California.
...
\begin{itemize}
    \item 77~\% only with wildcard
    \item does not affect MAC tracking
    \item from SSID list it is possible to get charasteristics of user (student, restaurants, hotels, hometown)
    \item but NOT possible to identify a single person, only in rare circumstances or with huge effort
    \item $->$ we think that SSID list should not be personal data in EU, nor ssid-geolocation databases
    \item but MAC address tracking should be, because of e.g. CCTV
    \item legal in US, but may change in the future -- however, unclear if SSID list is part of regulations
    \item again, 77~\% of users had wildcards $->$ SSID analysis by examining probes is not efficient, but the list can be obtained by other ways, e.g. android apps~\cite{2014_WISEC_Achara}
    \item countermeasures should be implemented! MAC randomization, wildcard probing, long-term: protocol changes
\end{itemize}
