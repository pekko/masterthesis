...
Our choice for database software was MongoDB~\cite{mongodb}, which is a high-performance NoSQL database. MongoDB is specifically designed to work with JSON format, so importing data was painless. It also includes native support for geolocation queries, which play important role when using the Twitter data.
...
For Twitter, we collected 10~\% of continuous feed in 2014 (January - August). After that, we filtered out all the results without geolocation data. In the end, we had 164 million tweets -- posts to Twitter -- in the database.
