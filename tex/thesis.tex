...
We collected total of 315,451 probe requests from 15,124 different devices. Here, we assume that devices are following the standard and have an unique and permanent MAC address. 11,215 devices (74.2~\%) probed only wildcard SSID, and these were excluded from later analysis. Therefore, we have 3,909 unique devices left.

We saw 9,181 unique SSIDs. Distribution of nodes per SSID is shown on Figure~\ref{fig:nodes_per_ssid}. We see that huge majority of SSIDs, 7,927 or 86~\%, were found only on a single device. This will make it harder to find clusters, but allows to differentiate users more.
