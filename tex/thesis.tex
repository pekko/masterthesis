...
Unfortunately even the same MAC address does not guarantee that AP is legitimate: MAC address can be spoofed. It is possible to construct two identical probe requests from two separate devices, thus the client needs to verify the authenticity of AP by other means.

When attacker is exploiting user's probe requests to guess a name of network user would be willing to connect to, defense is straightforward: do not send probe requests with SSIDs, but resort to wildcard probing or passive scanning. A problem with passive scanning is that it may reduce performance, which manufacturers do not like to see. However, active scanning with wildcard SSID should be fast enough.
